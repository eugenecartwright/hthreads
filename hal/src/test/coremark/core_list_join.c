...
list_head *core_list_mergesort(int flag, list_head *list, list_cmp cmp, core_results *res);
...
ee_u16 core_bench_list(core_results *res, ee_s16 finder_idx) {
	...
	if (finder_idx>0)
		list=core_list_mergesort(0,list,cmp_complex,res);
	...
	list=core_list_mergesort(1,list,cmp_idx,NULL);
    ...
    while (1) {
        ...
        while (p) {
            ...
            while (psize > 0 || (qsize > 0 && q)) {
                ...
                ee_s32 temp_compare = 0;
                //if (cmp == &cmp_idx)
                if (flag == 1)
                {
                    temp_compare = cmp_idx(p->info, q->info,res);
                }
                else
                {
                    temp_compare = cmp_complex(p->info, q->info,res);
                }
