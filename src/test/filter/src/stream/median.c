...
extern int median_win;
extern int median_run;
void* median_thread( void *arg )
{
    ...
    if( node->input == NULL )
    {
        fprintf(stderr,"median node must be used with a valid input buffer\n");
        exit(1 );
    }
    ...
    if( node->output == NULL )
    {
        fprintf(stderr,"median node must be used with a valid output buffer\n");
        exit(1 );
    }
    ...
    while( 1 )
    {
        ...
        if( dst != NULL && median_run )
        {
            // Perform the filter
            framebuffer_median( dst, frame, median_win );
