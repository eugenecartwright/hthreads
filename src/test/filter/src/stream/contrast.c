...
void* contrast_thread( void *arg )
{
    ...
    if( node->input == NULL )
    {
        fprintf(stderr,"contrast node must be used with a valid input buffer\n");
        exit(1 );
    }
    ...
    if( node->output == NULL )
    {
        fprintf(stderr,"contrast node must be used with a valid output buffer\n");
        exit(1 );
    }
    ...
    while( 1 )
    {
        ...
        if( dst != NULL && contrast_run )
        {
            // Perform the filter
            framebuffer_contrast( dst, frame );
