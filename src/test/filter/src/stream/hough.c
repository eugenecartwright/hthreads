...
extern int hough_run;
void* hough_thread( void *arg )
{
    int res;
    framebuffer_t *dst;
    framebuffer_t *frame;
    ...
    while( 1 )
    {
        ...
        if( dst != NULL && hough_run ) {


            // Perform the filter
            hough_width = 2*sqrt((frame->width)*(frame->width) + (frame->height)*(frame->height))+1;
            framebuffer_hough( dst, frame, hough_width );
