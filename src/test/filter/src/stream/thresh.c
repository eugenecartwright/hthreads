...
extern int thresh_run;
void* thresh_thread( void *arg )
{
    int res;
    framebuffer_t *dst;
    framebuffer_t *frame;
    ...
    if( node->input == NULL )
    {
        fprintf(stderr,"threshold node must be used with a valid input buffer\n");
        exit(1 );
    }
    ...
    if( node->output == NULL )
    {
        fprintf(stderr,"threshold node must be used with a valid output buffer\n");
        exit(1 );
    }
    ...
    while( 1 )
    {
        ...
        if( dst != NULL && thresh_run )
        {
            // Perform the filter
            framebuffer_thresh( dst, frame, thresh_val );
