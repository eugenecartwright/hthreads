...
void tgaoutput_check_buffers( stream_node_t *node )
{
...
void tgaoutput_capture_time( hthread_time_t *s, hthread_time_t *e, Huint *f )
{
    hthread_time_t diff;
    double sec;
...
void tgaoutput_cleanup_frame( stream_node_t *node, framebuffer_t *frame )
{
...
void tgaoutput_write_file( framebuffer_t *frame )
{
    framebuffer_tgaout( frame, "image.tga", TGA_RAW );
}

void* tgaoutput_thread( void *arg )
{
    framebuffer_t *frame;
    stream_node_t *node;
    ...
    while( 1 )
    {
        ...
        tgaoutput_write_file( frame );

        // Show the FPS that we are handling
        tgaoutput_capture_time( &start, &end, &frames );

        // Clean up the processed frame
        tgaoutput_cleanup_frame( node, frame );
    }
