...
typedef struct {
    ...
    unsigned char *imageData; // Binary data (RGB only for the encoder/decoder)
#ifdef HTHREADS_BUILD
    unsigned char *src_image; // Hex array (Raw BMP data)
    unsigned char *jpeg_data;
    pixel_t *dst_image; // RGB to display from the board
#endif
...
#else
void SaveJPEG(IplImage *dstImg, List_int*, FramePop, uint8_t[], uint8_t[]);
void LoadJPEG(IplImage *srcImg, List_int*, FramePop*, uint8_t[], uint8_t[]);
#endif
