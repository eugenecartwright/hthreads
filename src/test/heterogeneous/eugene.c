   ...
   for (j = 0; j < NUM_TRIALS; j++) 
   {
       ...
       ((thread_package+i)->arg4)++;
           ...
           if (thread_create (&child[i],
                       &attr[i],
                       (void *) foo_thread_FUNC_ID, 
                       (void *)(thread_package+i),
                       SOFTWARE_THREAD,
                       0)) 
           {
           ...
           //dynamic_create_smart(&child[i],&attr[i],foo_thread_FUNC_ID, (void *)(thread_package+i));
           //hthread_time_t stop = hthread_time_get();
           //hthread_time_t diff;
           //hthread_time_diff(diff,stop,start);
           //printf("Total OS overhead = %.6f us\n", hthread_time_usec(diff));
#ifdef  DYNAMIC
           //hthread_time_t start = hthread_time_get();
           thread_create(
                   &child[i],
       ...
       for (i = 0; i < NUM_AVAILABLE_HETERO_CPUS; i++) 
       {
           ...
           //if( thread_join(child[i], (void *) &ret[i], NULL, 0, 0))
           {
