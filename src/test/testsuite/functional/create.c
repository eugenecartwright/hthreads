	...
	while( 1 ) {
		...
		switch( rpc.opcode ) {
			case 0x8010:
				...
				rpc.result = hthread_create( &thread,
					NULL,
					foo, 
					NULL );
				break;
			...
			case 0x8011:
				...
				rpc.result = hthread_join( thread,
					NULL );
				break;
		}
...
int main(int argc, char *argv[]) {
	...
	hthread_attr_sethardware( &hwAttr, HWTI_ONE_BASEADDR );

	arg.attribute = &swAttr;
	arg.function = foo;
	arg.argument = NULL;

	printf( "Creating one hardware thread\n" );
	...
	printf( "Creating zero hardware thread\n" );
	hthread_attr_sethardware( &hwAttr, HWTI_ZERO_BASEADDR );
    hthread_create( &hwThread[1], &hwAttr, NULL, &arg );
