...
void * encode_data(void * arg)
{
   ...
   int BufLen = 0;
   int outputIndex = 0;
   int i, Index, Len, Pos, Code[MAX_SIZE];
...
//------------------------------------------------------------
void HuffEncode(int * inputData, int * outputData, int length)
{
   int Min, Max, Range, Index, *Histo, *ptr;
   struct HuffmanCode code;
   ...
   if ( HARDWARE ) hthread_attr_sethardware( &attr, HWTI_ZERO_BASEADDR );
...
int main() {
	...
	for( i=0; i<length; i++ ) inputData[i] = rand() % 256;
	
	log_create( &log, 1024 );
	HuffEncode( inputData, outputData, length );
	
	//for( i=0; i<32; i++ ) printf( "input  %x\n", inputData[i] );
	//for( i=0; i<32; i++ ) printf( "output  %x\n", outputData[i] );
	
	printf( "Size %d Hardware %d\n", LENGTH, HARDWARE );
	log_close_ascii( &log );

	if ( HARDWARE ) readHWTStatus( HWTI_ZERO_BASEADDR );
	return 0;
}
