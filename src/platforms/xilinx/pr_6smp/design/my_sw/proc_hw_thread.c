...
void initialize_interface( proc_interface_t * iface, int * baseAddr)
{
    ...
    iface->accelerator_flags        = (int*)(baseAddr + 22);
    ...
    *(iface->processor_type) = (int) TYPE_MB;
    ...
    // Register this slave as having an accelerator and PR capabilities
    //*(iface->accelerator_flags) |= (ACCELERATOR_FLAG | PR_FLAG);
