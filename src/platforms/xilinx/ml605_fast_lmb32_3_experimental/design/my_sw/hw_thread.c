...
proc_interface_t hwti __attribute__((section("vhwti")));
...
int main() {
    ...
    // Reset timer
    //if (((unsigned int) cpu_id % CPUS_PER_BUS) == 0) 
    ...
    check = (unsigned int *)
        ((unsigned int) check + ( ((unsigned int) cpu_id % CPUS_PER_BUS) *  0x100));
    ...
    *check = OKAY_VALUE;
    ...
    while(1) {
	    ...
	    // Wait to be "started"
	    wait_for_go_command((proc_interface_t *)&hwti);
