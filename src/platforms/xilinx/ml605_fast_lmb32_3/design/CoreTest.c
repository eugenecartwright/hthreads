...
static inline unsigned int _current_thread ( void )
{
    ...
    if( has_error(sta) )
    {
        xil_printf("_current_thread operation ERROR: 0x%08x\r\n", sta);
    }
...
static inline void _decode_sched_status( unsigned int status )
{
    ...
    xil_printf( "\tSched - (QUE=0x%02x) (NXT=0x%02x) (PRI=0x%02x)\r\n", queued, next, prior );
}
...
int main(){
    ...
    int j;
    {
        ...
        int j = 0, i = 0; volatile unsigned int * check = (unsigned int *) EXTRA_BRAM;
        for (i = 0; i < NUM_OF_SLAVES; i++) 
        {
            if(*check == OKAY_VALUE)  xil_printf("#%2d:ONLINE\t ", i);
            else                      xil_printf("#%2d:OFFLINE\t ", i);
            j++;
            if( j >= CPUS_PER_BUS ) 
            {
                j = 0;
                check = (unsigned int *) ( (unsigned int) check & 0xFFFFF000);
                check = (unsigned int *) ((unsigned int) check + 0x1000);
                xil_printf("\r\n");
            }
            else
